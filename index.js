...
setInterval(async () => {
  exchanges.forEach(async function (exchange) {
    if (!currentTickers[exchange.name]) {
      ...
      if (lastTicker.last < (currentTickers[exchange.name].last - lost)) {
      ...
      console.log(
                `${exchange.name}   Last: ${currentTickers[exchange.name].last.toFixed(2)}    Current: ${lastTicker.last.toFixed(2)}`
            )
      if (lastTicker.last > currentTickers[exchange.name].last) { currentTickers[exchange.name] = lastTicker }
      if (Date.parse(lastTicker.datetime) - Date.parse(currentTickers[exchange.name].datetime) > check) { currentTickers[exchange.name] = lastTicker }
    }
  })
}, interval)
